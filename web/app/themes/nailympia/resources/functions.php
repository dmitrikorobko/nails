...
if($_SERVER['SERVER_NAME'] === 'localhost' || $_SERVER['SERVER_NAME'] === 'localhost:3000') {
	// Replace src paths
	add_filter('wp_get_attachment_url', function ($url) {
		if(file_exists($url)) {
			return $url;
		}
		return str_replace(home_url(), 'https://nailympia.eu/', $url);
	});
	...
	add_filter('wp_calculate_image_srcset', function($sources) {
		foreach($sources as &$source) {
			if(!file_exists($source['url'])) {
				$source['url'] = str_replace(home_url(), 'https://nailympia.eu/', $source['url']);
			}
		}
		return $sources;
	});
